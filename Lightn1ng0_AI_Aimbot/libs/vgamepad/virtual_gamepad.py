...
from . import vigem_commons
from . import vigem_client
import ctypes
...
def check_err(err):
    if err != vigem_commons.VIGEM_ERRORS.VIGEM_ERROR_NONE:
        raise Exception(vigem_commons.VIGEM_ERRORS(err).name)
...
class VBus:
    ...
    """
    def __init__(self):
        self._busp = vigem_client.vigem_alloc()
        check_err(vigem_client.vigem_connect(self._busp))

    def get_busp(self):
        return self._busp

    def __del__(self):
        vigem_client.vigem_disconnect(self._busp)
        vigem_client.vigem_free(self._busp)
...
class VGamepad(ABC):
    def __init__(self):
        ...
        self.cmp_func = None
        vigem_client.vigem_target_add(self._busp, self._devicep)
        assert vigem_client.vigem_target_is_attached(self._devicep), "The virtual device could not connect to ViGEmBus."

    def __del__(self):
        vigem_client.vigem_target_remove(self._busp, self._devicep)
        vigem_client.vigem_target_free(self._devicep)

    def get_vid(self):
        """
        :return: the vendor ID of the virtual device
        """
        return vigem_client.vigem_target_get_vid(self._devicep)

    def get_pid(self):
        """
        :return: the product ID of the virtual device
        """
        return vigem_client.vigem_target_get_pid(self._devicep)

    def set_vid(self, vid):
        """
        :param: the new vendor ID of the virtual device
        """
        vigem_client.vigem_target_set_vid(self._devicep, vid)

    def set_pid(self, pid):
        """
        :param: the new product ID of the virtual device
        """
        vigem_client.vigem_target_get_pid(self._devicep, pid)

    def get_index(self):
        """
        :return: the internally used index of the target device
        """
        return vigem_client.vigem_target_get_index(self._devicep)

    def get_type(self):
        """
        :return: the type of the object (e.g. VIGEM_TARGET_TYPE.Xbox360Wired)
        """
        return vigem_client.vigem_target_get_type(self._devicep)

    @abstractmethod
    def target_alloc(self):
        """
        :return: the pointer to an allocated ViGEm device (e.g. vigem_client.vigem_target_x360_alloc())
        """
        pass
...
class VX360Gamepad(VGamepad):
    ...
    def get_default_report(self):
        return vigem_commons.XUSB_REPORT(
            wButtons=0,
            bLeftTrigger=0,
            bRightTrigger=0,
            sThumbLX=0,
    ...
    def update(self):
        ...
        """
        check_err(vigem_client.vigem_target_x360_update(self._busp, self._devicep, self.report))
    ...
    def register_notification(self, callback_function):
        ...
        check_err(vigem_client.vigem_target_x360_register_notification(self._busp, self._devicep, self.cmp_func, None))

    def unregister_notification(self):
        """
        Unregisters a previously registered callback function.
        """
        vigem_client.vigem_target_x360_unregister_notification(self._devicep)
...
class VDS4Gamepad(VGamepad):
    ...
    def get_default_report(self):
        rep = vigem_commons.DS4_REPORT(
            ...
            bTriggerL=0,
            bTriggerR=0)
        vigem_commons.DS4_REPORT_INIT(rep)
        return rep
    ...
    def directional_pad(self, direction):
        ...
        """
        vigem_commons.DS4_SET_DPAD(self.report, direction)

    def update(self):
        """
        Sends the current report (i.e. commands) to the virtual device
        """
        check_err(vigem_client.vigem_target_ds4_update(self._busp, self._devicep, self.report))
    ...
    def update_extended_report(self, extended_report):
        ...
        """
        check_err(vigem_client.vigem_target_ds4_update_ex_ptr(self._busp, self._devicep, ctypes.byref(extended_report)))
    ...
    def register_notification(self, callback_function):
        ...
        check_err(vigem_client.vigem_target_ds4_register_notification(self._busp, self._devicep, self.cmp_func, None))

    def unregister_notification(self):
        """
        Unregisters a previously registered callback function.
        """
        vigem_client.vigem_target_ds4_unregister_notification(self._devicep)

    def target_alloc(self):
        return vigem_client.vigem_target_ds4_alloc()
